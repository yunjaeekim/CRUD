...
/* =========================================================
 * DOM References
 * 화면 요소의 ID는 index.html과 동일한 이름으로 관리한다.
 * ========================================================= */
...
/* =========================================================
 * Application State
 * 별도 저장소를 사용하지 않으므로 새로고침 시 초기 데이터로 복원된다.
 * ========================================================= */
...
/*
 * null이면 신규 등록 모드다.
 * 차량 ID가 저장되어 있으면 해당 차량의 수정 모드다.
 */
let editingCarId = null

/* 페이지 최초 진입 시 초기 차량 목록을 출력한다. */
renderUsedCars()
...
/**
 * 현재 검색어와 판매 상태 조건을 적용하여 차량 목록을 다시 출력한다.
 *
 * 데이터 변경 후에는 이 함수를 호출하여 화면과 상태를 동기화한다.
 */
...
/**
 * 검색어와 판매 상태 조건을 모두 만족하는 차량을 반환한다.
 *
 * 검색어가 비어 있으면 모든 제조사와 모델명이 검색 조건을 통과한다.
 * 원본 배열은 변경하지 않으며 화면에 표시할 목록만 새로 생성한다.
 *
...
/**
 * 차량 한 대를 표시하는 카드 요소를 생성한다.
 *
 * 버튼에는 차량 ID와 작업 종류를 data 속성으로 저장한다.
 * 목록 영역의 이벤트 위임에서 이 값을 사용해 수정과 삭제를 구분한다.
 *
 * @param {object} car 차량 정보
 * @returns {HTMLElement} 화면에 추가할 차량 카드
 */
...
/**
 * 폼 입력값을 명세의 순서대로 검증하고 차량 객체로 반환한다.
 *
 * 검증에 실패하면 명세에 정의된 경고 메시지를 표시하고,
 * 사용자가 수정해야 할 입력 요소로 포커스를 이동한다.
 *
...
/**
 * 선택한 차량 정보를 입력 폼에 반영하고 수정 모드로 전환한다.
 *
...
/**
 * 사용자 확인 후 선택한 차량을 목록에서 제거한다.
 *
 * 수정 중인 차량을 삭제한 경우에는 폼도 신규 등록 상태로 복원한다.
 *
...
/* =========================================================
 * Event Handlers
 * 이벤트는 DOM 요소가 선언된 이후 한 곳에서 연결한다.
 * ========================================================= */

/*
 * 동일한 submit 이벤트에서 신규 등록과 수정 완료를 처리한다.
 * editingCarId 값으로 현재 모드를 구분한다.
 */
...
/*
 * 차량 카드는 목록을 다시 그릴 때마다 새로 생성된다.
 * 목록 컨테이너 하나에 이벤트를 연결하여 동적 버튼을 처리한다.
 */
...
/*
 * input은 일반 키보드 입력을 처리한다.
 * search는 Safari 검색 입력의 지우기 버튼 사용 시 목록 갱신을 보장한다.
 * change는 포커스 이동 시 최종 입력값을 한 번 더 반영한다.
 */
usedCarSearchInput.addEventListener('input', renderUsedCars)
usedCarSearchInput.addEventListener('search', renderUsedCars)
usedCarSearchInput.addEventListener('change', renderUsedCars)
